  ...
  "node_modules/toml/lib/compiler.js"(exports, module2) {
    ...
    function compile(nodes) {
      ...
      function setPath(node) {
        ...
        context = deepRef(data, path7, /* @__PURE__ */ Object.create(null), line, column);
        currentPath = path7;
      }
      function addTableArray(node) {
        ...
        assignedPaths.push(quotedPath);
        context = deepRef(data, path7, [], line, column);
  ...
  "node_modules/semver/classes/range.js"(exports, module2) {
    var Range7 = class {
      constructor(range, options) {
        options = parseOptions(options);
        if (range instanceof Range7) {
          if (range.loose === !!options.loose && range.includePrerelease === !!options.includePrerelease) {
            return range;
          } else {
            return new Range7(range.raw, options);
          }
        }
  ...
  "node_modules/glob/common.js"(exports) {
    ...
    function setopts(self, pattern, options) {
      ...
      if (!ownProp(options, "cwd"))
        ...
        self.cwd = path7.resolve(options.cwd);
        self.changedCwd = self.cwd !== cwd;
      }
      self.root = options.root || path7.resolve(self.cwd, "/");
      self.root = path7.resolve(self.root);
    ...
    function makeAbs(self, f) {
      var abs = f;
      if (f.charAt(0) === "/") {
        abs = path7.join(self.root, f);
      } else if (isAbsolute(f) || f === "") {
        abs = f;
      } else if (self.changedCwd) {
        abs = path7.resolve(self.cwd, f);
      } else {
        abs = path7.resolve(f);
      }
      if (process.platform === "win32")
    ...
    function childrenIgnored(self, path8) {
      if (!self.ignore.length)
        return false;
      return self.ignore.some(function(item) {
  ...
  "node_modules/rimraf/rimraf.js"(exports, module2) {
    ...
    var rmkids = (p, options, cb) => {
      ...
      options.readdir(p, (er, files) => {
        ...
        let errState;
        files.forEach((f) => {
          rimraf2(path7.join(p, f), options, (er2) => {
            if (errState)
              return;
            if (er2)
...
var require_tmp = __commonJS({
  "node_modules/tmp/lib/tmp.js"(exports, module2) {
    ...
    function _prepareTmpDirRemoveCallback(name, opts, sync) {
      const removeFunction = opts.unsafeCleanup ? rimraf2 : fs8.rmdir.bind(fs8);
    ...
    function _resolvePath(name, tmpDir) {
      ...
      if (sanitizedName.startsWith(tmpDir)) {
        return path7.resolve(sanitizedName);
      } else {
        return path7.resolve(path7.join(tmpDir, sanitizedName));
      }
    }
    ...
    function _getTmpDir(options) {
      return path7.resolve(_sanitizeName(options && options.tmpdir || os.tmpdir()));
    }
...
var HtmlDjangoCodeActionProvider = class {
  constructor() {
    const extConfig = import_coc.workspace.getConfiguration("htmldjango");
  ...
  async provideCodeActions(document, range, context) {
    ...
    if (this.formattingProvider === "djhtml") {
      if (this.lineRange(range)) {
        ...
        const edit = import_coc.TextEdit.insert(import_coc.Position.create(range.start.line, suppressLineLength), suppressLineNewText);
        codeActions.push({
...
var FiltersSnippetsCompletionProvider = class {
  constructor(context) {
    ...
    this.snippetsFilePath = import_path.default.join(this._context.extensionPath, "snippets", "filters.json");
    this.excludeSnippetsKeys = import_coc2.workspace.getConfiguration("htmldjango").get("completion.exclude", []);
  }
  async getSnippetsCompletionItems(snippetsFilePath) {
    const snippetsCompletionList = [];
    if (import_fs.default.existsSync(snippetsFilePath)) {
      const snippetsJsonText = import_fs.default.readFileSync(snippetsFilePath, "utf8");
...
var TagsSnippetsCompletionProvider = class {
  constructor(context) {
    this._context = context;
    this.snippetsFilePath = import_path2.default.join(this._context.extensionPath, "snippets", "tags.json");
    this.excludeSnippetsKeys = import_coc3.workspace.getConfiguration("htmldjango").get("completion.exclude", []);
  }
  async getSnippetsCompletionItems(snippetsFilePath) {
    const snippetsCompletionList = [];
...
function getPackageVersion(name) {
  const rootDir = import_path3.default.resolve(import_path3.default.dirname(__filename), "..");
  const filePath = import_path3.default.join(rootDir, "pyproject.toml");
  const fileStr = import_fs3.default.readFileSync(filePath);
  const data = import_toml.default.parse(fileStr.toString());
  const version = data.tool.poetry.dependencies[name];
  return version;
}
var DJHTML_VERSION = getPackageVersion("djhtml");
var DJLINT_VERSION = getPackageVersion("djlint");
...
function resolveDjhtmlPath(context, toolPath) {
  if (!toolPath) {
    ...
    } else if (import_fs4.default.existsSync(import_path4.default.join(context.storagePath, "htmldjango", "venv", "Scripts", "djhtml.exe")) || import_fs4.default.existsSync(import_path4.default.join(context.storagePath, "htmldjango", "venv", "bin", "djhtml"))) {
      if (process.platform === "win32") {
        toolPath = import_path4.default.join(context.storagePath, "htmldjango", "venv", "Scripts", "djhtml.exe");
      } else {
        toolPath = import_path4.default.join(context.storagePath, "htmldjango", "venv", "bin", "djhtml");
      }
    } else {
      toolPath = "";
    }
...
function resolveDjlintPath(context, toolPath) {
  if (!toolPath) {
    ...
    } else if (import_fs4.default.existsSync(import_path4.default.join(context.storagePath, "htmldjango", "venv", "Scripts", "djlint.exe")) || import_fs4.default.existsSync(import_path4.default.join(context.storagePath, "htmldjango", "venv", "bin", "djlint"))) {
      if (process.platform === "win32") {
        toolPath = import_path4.default.join(context.storagePath, "htmldjango", "venv", "Scripts", "djlint.exe");
      } else {
        toolPath = import_path4.default.join(context.storagePath, "htmldjango", "venv", "bin", "djlint");
      }
    } else {
      toolPath = "";
    }
...
async function doDjhtmlFormat(context, outputChannel, document, range) {
  ...
  if (!djhtmlPath) {
    import_coc4.window.showErrorMessage('Unable to find the "djhtml" command.');
    return originalText;
  }
...
async function doDjlintFormat(context, outputChannel, document, range) {
  ...
  const extensionConfig = import_coc5.workspace.getConfiguration("htmldjango");
  const indentLevel = extensionConfig.get("djlint.indent", 4);
  const profile = extensionConfig.get("djlint.profile", "django");
  const preserveLeadingSpace = extensionConfig.get("djlint.preserveLeadingSpace", false);
  const preserveBlankLines = extensionConfig.get("djlint.preserveBlankLines", false);
  const formatCss = extensionConfig.get("djlint.formatCss", false);
  const formatJs = extensionConfig.get("djlint.formatJs", false);
  const ignoreCase = extensionConfig.get("djlint.ignoreCase", false);
  const ignoreBlocks = extensionConfig.get("djlint.ignoreBlocks", "");
  const addFormatterArgs = extensionConfig.get("djlint.addFormatterArgs", []);
  const text = document.getText(range);
  const fileName = import_coc5.Uri.parse(document.uri).fsPath;
  let djlintPath = extensionConfig.get("djlint.commandPath", "");
  djlintPath = resolveDjlintPath(context, djlintPath);
  if (!djlintPath) {
    import_coc5.window.showErrorMessage('Unable to find the "djlint" command.');
    return text;
  }
  let toolVersion;
  ...
  const args = [];
  const cwd = import_coc5.Uri.file(import_coc5.workspace.root).fsPath;
  const opts = { cwd, shell: true };
  args.push("--reformat");
  ...
  if (toolVersion && import_semver.default.gte(toolVersion, "1.2.0")) {
    if (preserveLeadingSpace)
      args.push("--preserve-leading-space");
  }
  if (toolVersion && import_semver.default.gte(toolVersion, "1.3.0")) {
    if (preserveBlankLines)
      args.push("--preserve-blank-lines");
  }
  if (toolVersion && import_semver.default.gte(toolVersion, "1.9.0")) {
    if (formatCss)
      args.push("--format-css");
    if (formatJs)
      args.push("--format-js");
  }
  if (toolVersion && import_semver.default.gte(toolVersion, "1.23.0")) {
    if (ignoreCase)
      args.push("--ignore-case");
  }
  if (toolVersion && import_semver.default.gte(toolVersion, "1.24.0")) {
    if (ignoreBlocks)
      args.push("--ignore-blocks", `'${ignoreBlocks}'`);
  }
  if (addFormatterArgs) {
    args.push(addFormatterArgs.join(" "));
  }
  const tmpFile = import_tmp.default.fileSync();
  import_fs5.default.writeFileSync(tmpFile.name, text);
...
`);
  ...
  return new Promise(function(resolve) {
    import_child_process3.default.execFile(djlintPath, [...args, tmpFile.name], opts, function(error, stdout, stderr) {
      let updateText;
      const originalText = import_fs5.default.readFileSync(tmpFile.name, "utf-8");
      if (error) {
...
`);
        if (isSuccess) {
          updateText = import_fs5.default.readFileSync(tmpFile.name, "utf-8");
        } else {
...
function fullDocumentRange(document) {
  ...
  return import_coc6.Range.create({ character: 0, line: 0 }, { character: doc.getline(lastLineId).length, line: lastLineId });
}
var HtmlDjangoFormattingEditProvider = class {
  ...
  getFormatFunc(formatter) {
    ...
    if (formatter) {
      ...
      const extensionConfig = import_coc6.workspace.getConfiguration("htmldjango");
      const formattingProvider = extensionConfig.get("formatting.provider", "djlint");
  ...
  async _provideEdits(document, range) {
    ...
    return [import_coc6.TextEdit.replace(range, code)];
  }
};
  ...
  async getHover(text, hoverLang) {
    ...
    for (const h in defineHovers) {
      if (text === defineHovers[h].prefix || defineHovers[h].alias.includes(text)) {
        const markdownPath = import_path5.default.join(
...
async function installTools(pythonCommand, context) {
  const pathVenv = import_path6.default.join(context.storagePath, "htmldjango", "venv");
  let pathVenvPython = import_path6.default.join(context.storagePath, "htmldjango", "venv", "bin", "python");
  if (process.platform === "win32") {
    pathVenvPython = import_path6.default.join(context.storagePath, "htmldjango", "venv", "Scripts", "python");
  }
  const statusItem = import_coc8.window.createStatusBarItem(0, { progress: true });
  ...
  try {
    import_coc8.window.showInformationMessage(`Install htmldjango related tools...`);
    await exec2(installCmd);
    statusItem.hide();
    import_coc8.window.showInformationMessage(`htmldjango: Installation is complete!`);
  } catch (error) {
    statusItem.hide();
    import_coc8.window.showErrorMessage(`htmldjango: Installation has failed. | ${error}`);
    throw new Error();
  }
}
...
var LintEngine = class {
  constructor(toolPath, toolVersion, outputChannel) {
    this.collection = import_coc9.languages.createDiagnosticCollection("djlint");
  ...
  async lint(textDocument) {
    if (!SUPPORT_LANGUAGES.includes(textDocument.languageId))
      return;
    const self = this;
    const filePath = import_coc9.Uri.parse(textDocument.uri).fsPath;
    const args = [];
    const cwd = import_coc9.Uri.file(import_coc9.workspace.root).fsPath;
    const opts = { cwd, shell: true };
    const extensionConfig = import_coc9.workspace.getConfiguration("htmldjango");
    const includeRules = extensionConfig.get("djlint.include", "");
    const ignoreRules = extensionConfig.get("djlint.ignore", "");
    const profile = extensionConfig.get("djlint.profile", "django");
    const useGitIgnore = extensionConfig.get("djlint.useGitIgnore", false);
    const preserveLeadingSpace = extensionConfig.get("djlint.preserveLeadingSpace", false);
    const preserveBlankLines = extensionConfig.get("djlint.preserveBlankLines", false);
    const addLinterArgs = extensionConfig.get("djlint.addLinterArgs", []);
    if (this.toolVersion && import_semver2.default.gte(this.toolVersion, "1.20.0")) {
      if (includeRules) {
    ...
    if (this.toolVersion && import_semver2.default.gte(this.toolVersion, "1.2.0")) {
      if (preserveLeadingSpace)
        args.push("--preserve-leading-space");
    }
    if (this.toolVersion && import_semver2.default.gte(this.toolVersion, "1.3.0")) {
      if (preserveBlankLines)
        args.push("--preserve-blank-lines");
    }
    if (addLinterArgs) {
...
`);
        ...
        if (djlintDiagnostics && djlintDiagnostics.length > 0) {
          for (const d of djlintDiagnostics) {
            ...
            const startPosition = import_coc9.Position.create(startLine, startCharacter);
            const endPosition = import_coc9.Position.create(endLine, endCharacter);
            const severity = self.getSeverity(d.ruleName);
            diagnostics.push({
              range: import_coc9.Range.create(startPosition, endPosition),
              message: d.message,
              severity,
              source: "djlint",
              code: d.ruleName,
...
`);
  ...
  getSeverity(s) {
    ...
    let severityNumber;
    if (s.startsWith("W")) {
      severityNumber = import_coc9.DiagnosticSeverity.Warning;
    }
    if (s.startsWith("E")) {
      severityNumber = import_coc9.DiagnosticSeverity.Error;
    }
    if (s.startsWith("T")) {
      severityNumber = import_coc9.DiagnosticSeverity.Warning;
    }
    ...
    switch (severityNumber) {
      case 1:
        severity = import_coc9.DiagnosticSeverity.Error;
        break;
      case 2:
        severity = import_coc9.DiagnosticSeverity.Warning;
        break;
      default:
        severity = import_coc9.DiagnosticSeverity.Warning;
        break;
...
async function activate(context) {
  ...
  context.subscriptions.push(
    import_coc10.commands.registerCommand("htmldjango.showOutput", () => {
      if (outputChannel) {
        outputChannel.show();
      }
    })
  ...
  context.subscriptions.push(
    import_coc10.commands.registerCommand("htmldjango.showReferences", async () => {
      const { document, position } = await import_coc10.workspace.getCurrentState();
      if (document.languageId !== "htmldjango")
        return;
      function parse3(template, openingAndClosingTags) {
        try {
          const parsed = mustache_default.parse(template, openingAndClosingTags);
          return parsed.map((p) => {
            return {
      ...
      const variablesBlock = parse3(text, ["{{", "}}"]);
      if (variablesBlock) {
        parsedBlocks.push(...variablesBlock);
      }
      const templateTagsBlock = parse3(text, ["{%", "%}"]);
      if (templateTagsBlock) {
        parsedBlocks.push(...templateTagsBlock);
      }
      const refs = [];
      parsedBlocks.filter((p) => p.templateSpanType === "name").forEach((p) => {
      ...
      import_coc10.commands.executeCommand(
        "editor.action.showReferences",
        import_coc10.Uri.parse(document.uri),
        position,
        refs.map(
          (ref) => import_coc10.Location.create(
            document.uri,
            import_coc10.Range.create(import_coc10.Position.create(ref.startLine, ref.startChar), import_coc10.Position.create(ref.endLine, ref.endChar))
          )
        )
      );
    })
  ...
  context.subscriptions.push(
    import_coc10.commands.registerCommand("htmldjango.builtin.installTools", async () => {
  ...
  if (formattingProvider === "djhtml" || formattingProvider === "djlint") {
    let registerFormatter2 = function() {
      ...
      rangeFormatterHandler = import_coc10.languages.registerDocumentRangeFormatProvider(
        rangeLanguageSelector,
        editProvider,
        priority
      );
  ...
  context.subscriptions.push(
    import_coc10.commands.registerCommand("htmldjango.djhtml.format", async () => {
  ...
  context.subscriptions.push(
    import_coc10.commands.registerCommand("htmldjango.djlint.format", async () => {
      const doc = await import_coc10.workspace.document;
  ...
  if (djlintPath && djlintEnableLint) {
    ...
    if (onChange) {
      import_coc10.workspace.onDidChangeTextDocument(
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        async (_e) => {
          const doc = await import_coc10.workspace.document;
          await engine.lint(doc.textDocument);
    ...
    if (onSave) {
      import_coc10.workspace.onDidSaveTextDocument(
        async (e) => {
          await engine.lint(e);
        },
        null,
  ...
  if (isEnableCompletion) {
    context.subscriptions.push(
      import_coc10.languages.registerCompletionItemProvider(
        "htmldjango-tags",
        "DJTags",
        ["htmldjango"],
...
async function installWrapper(pythonCommand, context) {
  ...
  const ret = await import_coc10.window.showPrompt(msg);
  if (ret) {
    try {
